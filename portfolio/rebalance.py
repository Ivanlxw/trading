...
class Rebalance(metaclass=ABCMeta):
    ...
    def need_rebalance(self, current_holdings):
        raise NotImplementedError("Should implement need_rebalance(). If not required, just pass")
    ...
    def rebalance(self, mkt_data, current_holdings, event_queue) -> None:
        ...
        if current_holdings[symbol].net_pos > 0:
            event_queue.appendleft(
                OrderEvent(
                    symbol,
                    current_holdings["datetime"],
                    current_holdings[symbol].net_pos,
                    OrderPosition.SELL,
                    OrderType.MARKET,
                    latest_close_price,
                )
            )
        elif current_holdings[symbol].net_pos < 0:
            event_queue.appendleft(
                OrderEvent(
                    symbol,
                    current_holdings["datetime"],
...
class RebalanceHalfYearly(PeriodicRebalance):
    ...
    def need_rebalance(self, current_holdings):
        return (
            current_holdings["datetime"].month % 6 == 1
            and current_holdings["datetime"].day < 7
...
class SellLosers(Rebalance, metaclass=ABCMeta):
    ...
    def rebalance(self, mkt_data, current_holdings, event_queue) -> None:
        ...
        elif current_holdings[symbol].net_pos < 0 and latest_close_price > current_holdings[
            symbol
        ].average_trade_price * (1 + self.perc):
            event_queue.appendleft(
                OrderEvent(
        ...
        if current_holdings[symbol].net_pos < 0:
            ...
            event_queue.appendleft(
                OrderEvent(
                    symbol,
                    current_holdings["datetime"],
                    abs(current_holdings[symbol].net_pos),
                    OrderPosition.BUY,
                    OrderType.MARKET,
                    latest_close_price,
                )
...
class StopLossThreshold(Rebalance):
    ...
    def rebalance(self, mkt_data, current_holdings, event_queue) -> None:
        ...
        if average_trade_price is None or current_holdings[symbol].net_pos == 0:
            return
        curr_qty = current_holdings[symbol].net_pos
...
class SellLowestPerforming(Rebalance):
    ...
    def signal_least_performing(self, universe_symbol_mkt_data_map, current_holdings):
        ...
        for sym in universe_symbol_mkt_data_map:
            ...
            if last_traded_price is None or current_holdings[sym].net_pos <= 0:
                continue
    ...
    def _filter_order_to_send(self, signal: SignalEvent, qty: int):
        ...
        if signal.order_position == OrderPosition.BUY:
            ...
            if self.current_holdings[sym].net_pos < 0 and sym != "":
                return [
                    OrderEvent(sym, latest_snapshot['datetime'][-1],
                               fabs(self.current_holdings[sym].net_pos), direction=OrderPosition.SELL, order_type=OrderType.MARKET, price=signal.price),
            ...
            if self.current_holdings[signal.symbol].net_pos < 0:
                if market_value < self.current_holdings["cash"]:
                    return [OrderEvent(signal.symbol, latest_snapshot['datetime'][-1],
                                       fabs(self.current_holdings[signal.symbol].net_pos), OrderPosition.BUY, self.order_type, price=signal.price)]
                sym = self.signal_least_performing()
                if sym != "":
                    return [
                        OrderEvent(sym, latest_snapshot['datetime'][-1],
                                   fabs(self.current_holdings[sym].net_pos), OrderPosition.SELL, OrderType.MARKET, price=signal.price),
                        OrderEvent(signal.symbol, latest_snapshot['datetime'][-1],
                                   fabs(self.current_holdings[signal.symbol].net_pos), OrderPosition.BUY, self.order_type, price=signal.price),
                    ]
            ...
            if self.current_holdings[signal.symbol].net_pos > 0:
                return [OrderEvent(signal.symbol, latest_snapshot['datetime'][-1],
                                   fabs(self.current_holdings[signal.symbol].net_pos), OrderPosition.SELL, self.order_type, price=signal.price)]
