...
def _ema(arr, period):
    ...
    for close_val in arr:
        curr_value = close_val * smoothing_weight + curr_value * \
            (1 - smoothing_weight) if not np.isnan(curr_value) else close_val
...
def candlestick_trade_pressure(open, close, high, low, num_trades):
    ...
    if avg_trades_per_period == 0:
        return 0
    impulse = np.fromiter((h - l if c - o > 0 else l - h for o,
                          c, h, l in zip(open, close, high, low)), dtype='float')
    return np.sum(impulse * num_trades / avg_trades_per_period)


def trade_weighted_candlestick_size(high, low, num_trades):
    return np.average(np.fromiter((h - l for h, l in zip(high, low)), dtype='float'), weights=num_trades / num_trades.sum())
...
def trade_weighted_signed_candlestick_size(high, low, open, close, num_trades):
    ...
    signed_iter = (_signed_candlestick_size(h, l, o, c)
                   for h, l, o, c in zip(high, low, open, close))
...
class TradeImpulseBase(Feature):
    ...
    def get_trade_pressure_base(self, data):
        return _trade_impulse_base(data['open'][-self.period:], data['close'][-self.period:], data['num_trades'][-self.period:])
...
class TradePressureEma(Feature):
    ...
    def get_trade_pressure_ema(self, data):
        val = trade_pressure_ema(data['open'][-self.period:], data['close']
                                 [-self.period:], data['volume'][-self.period:], self.period)  # data['num_trades'][-self.period:] / 
        '''
...
class TradePressureMaxEma(Feature):
    ...
    def get_trade_pressure_max_ema(self, data):
        val = _trade_pressure_max_ema(data['open'][-self.period:], data['close']
                                      [-self.period:], data['high'][-self.period:], data['low']
                                      [-self.period:],  data['volume'][-self.period:] / data['num_trades'][-self.period:], self.period)
        return val
...
class RSIDiffTradeWeighted(Feature):
    ...
    def get_trade_weighted_diff_rsi(self, data):
        ...
        candlestick_size = trade_weighted_candlestick_size(
            data['high'][-self.relative_period:], data['low'][-self.relative_period:], data['num_trades'][-self.relative_period:])
        simple_avg_rsi = np.average(rsis)
        multiplier = rsis[-1] / simple_avg_rsi
        multiplier = (multiplier ** 2 if multiplier >
                      1 else -1 * multiplier ** -2) - 1.0
