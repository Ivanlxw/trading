...
class SkLearnRegModel(Strategy):
    ...
    def _transform_data(self, ohlc_data, rebalance=False):
        ...
        X = pd.concat(
            [
                pd.Series(f._formula(ohlc_data), index=ohlc_data["datetime"], name=f.__class__.__name__)
...
class SkLearnRegModelNormalized(SkLearnRegModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            bars, event_queue, reg, features, target, rebalance_cond, order_val, n_history, freq, live, **kwargs
        )
