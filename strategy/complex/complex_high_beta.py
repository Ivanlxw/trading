...
class ComplexHighBeta(ComplexStrategyImpl):
  def __init__(self, bars: DataHandler, events: queue.Queue, index_syms: List[str], index_strategy: Strategy, corr_days:int, corr_min = 0.5, description: str = ""):
      ...
      assert all(x in bars.symbol_list for x in index_syms) , "Strategy needs index to be in symbol_list"
      assert corr_days > 25, "days to compare corr and beta has to be > 25 to be relatively robust"
  ...
  def _calculate_signal(self, _) -> List[SignalEvent]:
    ...
    for index_sym in self.index_symbols:
      ...
      if index_signal is None or len(index_signal) == 0:
        continue
      assert len(index_signal) == 1, f"{index_sym} signal should be the only one"
  ...
  def _calculate_internal(self) -> List[Tuple]:
      """ Returns a list tuples of reflecting (symbol, beta) beyond a specific correlation """
      ...
      for sym in self.bars.symbol_list:
        ...
        if len(snp_hist_prices["close"]) != len(sym_hist_prices["close"]):
          continue
        sym_index_corr =  np.corrcoef(snp_hist_prices["close"], sym_hist_prices["close"])
        if sym_index_corr[0][1] < self.corr_min:
          continue
        spy_pct_changes = self._helper_pct_change(snp_hist_prices["close"])
        sym_hist_changes = self._helper_pct_change(sym_hist_prices["close"])
        try:
          beta = np.cov(sym_hist_changes, spy_pct_changes)[0][1] / np.var(spy_pct_changes)
        except Exception as e:
          print(f"Input data that gave err: \n{sym}: {sym_hist_changes}\nSPY: {spy_pct_changes}")
