...
class ComplexHighBeta(ComplexStrategyImpl):
    ...
    def _calculate_internal(self) -> List[Tuple]:
        ...
        for sym in self.bars.symbol_list:
            ...
            if (
                len(snp_hist_prices["close"]) != len(sym_hist_prices["close"])
                or len(sym_hist_prices["close"]) < self.corr_days
            ):
                continue
            ...
            if spy_var <= 0:
                continue
            beta = np.cov(sym_hist_changes, spy_pct_changes)[0][1] / spy_var
