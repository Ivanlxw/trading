...
class PlotIndividual(Plot):
    ...
    def __init__(
        self, strategy: Strategy, signals: List[SignalEvent], plot_fair_prices: bool, historical_fair_prices: dict
    ) -> None:
        sns.set()
        sns.set_style("darkgrid")
    ...
    def plot(self):
        ...
        for idx, ticker in enumerate(tickers):
            ...
            if len(self.signals) > 0:
                ...
                fig.append_trace(
                    go.Scatter(
                        ...
                        mode="markers",
                        marker=dict(line=dict(width=2, color="blue"), symbol="x"),
                    ),
                    row=row,
                    col=col,
                )
                fig.append_trace(
                    go.Scatter(
                        ...
                        mode="markers",
                        marker=dict(color="black", line_width=2, symbol="x"),
                    ),
                    row=row,
                    col=col,
                )
            ...
            if self.plot_fair_prices:
                ...
                fig.append_trace(
                    go.Scatter(x=df.index, y=df["fair_ask"], line=dict(color="orange", width=1.5)), row=row, col=col
                )
